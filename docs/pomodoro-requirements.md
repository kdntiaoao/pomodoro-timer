...
- 既に `denied` の場合の再要求はブラウザ側で抑止される(ユーザーがブラウザ設定で許可し直す必要あり、UI 側で強制再要求はしない)
- 通知メッセージ:
  - 作業終了: タイトル「作業セッション完了」、本文「休憩を始めよう」
...
- **Web Audio API で生成**(`OscillatorNode` + `GainNode` による短い beep、約 0.5 秒)
  ...
  - 周波数 880Hz、attack/decay envelope で耳触り軽減
- 1 回のみ再生
- 音量調整スライダー(0〜100)
- ミュートは音量 0 で代替(独立トグルなし)
- 音量 0 の場合は `OscillatorNode` を生成しない(早期 return)
...
- メイン画面ヘッダー右上の「履歴」アイコン (`@hugeicons` の chart アイコン) から Dialog で開く
- Dialog 内構成: 上段に過去 7 日の棒グラフ、下段に直近 13 週のヒートマップ
- 空状態: セッションが 0 件の場合「まだ完了したセッションがない」旨を中央表示
